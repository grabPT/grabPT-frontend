...
export const detailInfoSchema = z.object({
  purpose: z
    ...
    .min(1, '목적을 하나 이상 선택해주세요.'),
  etcPurposeContent: z
    .string()
    .max(100, '기타 목적 입력은 100자 이하로 입력해주세요.')
    .optional(),
  ageGroup: z
  ...
  startPreference: z
    .string()
    .nonempty('시작 희망일을 입력해주세요.'),
  availableDays: z
    .array(z.enum(DAYS))
    .min(1, '가능한 요일을 하나 이상 선택해주세요.'),
  availableTimes: z
    .array(z.enum(TIMES))
    .min(1, '가능한 시간대를 하나 이상 선택해주세요.'),
  content: z
    .string()
    .max(500, '요청사항은 300자 이내로 입력해주세요.'),
});